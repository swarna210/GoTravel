    ...
    <SafeAreaView className="flex-1 bg-white relative">
        <ScrollView className="flex-1 px-4 py-6">
            <View className="relative bg-white shadow-lg">
                ...
                <TouchableOpacity className="w-10 h-10 rounded-md items-center justify-center bg-[#06B2BE]">
                    <FontAwesome5 name="heartbeat" size={24} color="#fff" onPress={()=>Alert.alert("You liked")} />
                </TouchableOpacity>
                </View>
          ...
          <View className="flex-row items-center space-x-2 mt-2">
            <FontAwesome name="map-marker" size={16} color="#8C9EA6" />
        ...
        {data?.phone && (
            <View className="items-center flex-row space-x-6">
              ...
              <Text className="text-[16px]">{data?.phone}</Text>
            </View>
          )}
          {data?.email && (
            <View className="items-center flex-row space-x-6">
              <FontAwesome name="envelope" size={24} color="#428288" />
              <Text className="text-[16px]">{data?.email}</Text>
            </View>
          )}
           {data?.address && (
            <View className="items-center flex-row space-x-6">
              <FontAwesome name="map-pin" size={24} color="#428288" />
              <Text className="text-[16px]">{data?.address}</Text>
            </View>
          )}


<View className="mt-4 py-3 rounded-lg bg-[#06B2BE] items-center justify-center mb-12">
    <TouchableOpacity onPress={()=>Alert.alert('Booking successfull')}>
